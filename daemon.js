import {
    ...
    hashCode, disableLogs, log, getFilePath, initScriptPaths, getConfiguration,
...
export async function main(ns) {
    // Load script-paths.json so getFilePath() resolves reorganized paths (0 GB)
    initScriptPaths(ns);
    // --- CONSTANTS ---
    ...
    async function startup(ns) {
        ...
        asynchronousHelpers = [
            ...
            { name: "sleeve/sleeve.js", shouldRun: () => reqRam(64) && 10 in dictSourceFiles }, // Script to create manage our sleeves for us
            { name: "gangs/gangs.js", shouldRun: () => reqRam(64) && 2 in dictSourceFiles }, // Script to create manage our gang for us
            {
            ...
            {
                name: "bladeburner/bladeburner.js", // Script to manage bladeburner for us. Run automatically if not disabled and bladeburner API is available
                shouldRun: () => !options['disable-script'].includes('bladeburner/bladeburner.js') && reqRam(64)
        ...
        do {
            ...
            try {
                ...
                if (xpOnly) { // If all we want to do is gain hack XP
                    ...
                    !processList(ns, daemonHost).some(p => p.filename.endsWith('hacking/hwgw-manager.js'))) {
    ...
    async function updateStockPositions(ns) {
        ...
        Object.keys(serverStockSymbols).forEach(server => {
            ...
            newShouldManipulateGrow[server] = pos.sharesLong > 0 || pos.prob >= 0.5; // If bullish, grow should be made to influence stock
            newShouldManipulateHack[server] = pos.sharesShort > 0 || pos.prob < 0.5; // If bearish, hack should be made to influence stock
