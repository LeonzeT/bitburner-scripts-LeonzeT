...
export async function main(ns) {
    ...
    log(ns, `INFO: Max purchasable RAM has been detected as 2^${maxPurchasableServerRamExponent} (${ns.format.ram(2 ** maxPurchasableServerRamExponent)}).`);
    ...
    if (minRamExponent > maxPurchasableServerRamExponent) {
        log(ns, `WARN: --min-ram-exponent was set to ${minRamExponent} (${ns.format.ram(2 ** minRamExponent)}), ` +
            `but the maximum server RAM in this BN is ${maxPurchasableServerRamExponent} (${ns.format.ram(2 ** maxPurchasableServerRamExponent)}), ` +
        ...
        log(ns, `INFO: --min-ram-exponent is set to ${minRamExponent}: New servers will only be purchased ` +
            `if we can afford 2^${minRamExponent} (${ns.format.ram(2 ** minRamExponent)}) or more in size.`);
...
async function tryToBuyBestServerPossible(ns) {
    ...
    if (likelyHacknet.length > 0) {
        ...
        if (totalHacknetUsedRam == 0) {
            ...
            log(ns, `We are currently using ${ns.format.ram(totalHacknetUsedRam)} of hacknet RAM, so including hacknet in our utilization stats.`)
    }
    ...
    if (options['utilization-trigger'] > 0) {
        ...
        setStatus(ns, `Using ${Math.round(totalUsedRam).toLocaleString('en')}/${ns.format.ram(totalMaxRam)} (` +
    ...
    if (purchasedServers.length >= maxPurchasedServers) {
        if (worstServerRam == maxPurchasableServerRam) {
            ...
            return setStatus(ns, `INFO: We are at the max number of servers ${maxPurchasedServers}, ` +
                `and all have the maximum possible RAM (${ns.format.ram(maxPurchasableServerRam)}).`);
        }
    ...
    if (!purchasedServer)
        setStatus(ns, `${prefix}Could not ${isUpgrade ? 'upgrade' : 'purchase'} a server with ${ns.format.ram(maxRamPossibleToBuy)} ` +
            `RAM for ${formatMoney(cost)} with a budget of ${formatMoney(spendableMoney)}.`);
    else {
        log(ns, `SUCCESS: ${isUpgrade ? 'Upgraded' : 'Purchased'} server ${purchasedServer} with ${ns.format.ram(maxRamPossibleToBuy)} ` +
