...
const argsSchema = [
    ...
    ['training-cap-seconds', 24 * 60 * 60 /* 24 hours */], // Time since the start of the bitnode after which we will no longer attempt to train sleeves to their target "train-to" settings
...
/** Converts a dashboard taskSpec into a [designatedTask, command, args, statusUpdate] tuple. */
function buildOverrideTask(i, spec) {
    switch (spec.type) {
        case 'CRIME':
            return [`commit ${spec.crimeType}`,
                `ns.sleeve.setToCommitCrime(ns.args[0], ns.args[1])`,
                [i, spec.crimeType],
                `committing ${spec.crimeType} (dashboard override)`];
        case 'RECOVERY':
            return [`recover from shock`,
                `ns.sleeve.setToShockRecovery(ns.args[0])`,
                [i],
                `recovering from shock (dashboard override)`];
        case 'SYNCHRO':
            return [`synchronize`,
                `ns.sleeve.setToSynchronize(ns.args[0])`,
                [i],
                `synchronizing (dashboard override)`];
        case 'FACTION':
            return [`work for faction '${spec.factionName}' (${spec.workType})`,
                `ns.sleeve.setToFactionWork(ns.args[0], ns.args[1], ns.args[2])`,
                [i, spec.factionName, spec.workType],
                `working for ${spec.factionName} (dashboard override)`];
        case 'COMPANY':
            return [`work for company '${spec.companyName}'`,
                `ns.sleeve.setToCompanyWork(ns.args[0], ns.args[1])`,
                [i, spec.companyName],
                `working for ${spec.companyName} (dashboard override)`];
        case 'CLASS':
            return [`study ${spec.course} (${spec.university})`,
                `ns.sleeve.setToUniversityCourse(ns.args[0], ns.args[1], ns.args[2])`,
                [i, spec.university, spec.course],
                `studying ${spec.course} at ${spec.university} (dashboard override)`];
        case 'GYM':
            return [`train ${spec.stat} (${spec.gym})`,
                `ns.sleeve.setToGymWorkout(ns.args[0], ns.args[1], ns.args[2])`,
                [i, spec.gym, spec.stat],
                `training ${spec.stat} at ${spec.gym} (dashboard override)`];
        default:
            return [`idle`, `ns.sleeve.setToShockRecovery(ns.args[0])`, [i], `unknown override type ${spec.type}`];
    }
}
...
async function mainLoop(ns) {
    ...
    // Time cap is a backstop to prevent infinite training when mults are crippling and targets
    // can never be reached. It does NOT apply if a sleeve is still below its target — the
    // per-sleeve untrainedStats check inside pickSleeveTask handles that correctly.
    const trainingCapExpired = effectiveTrainingCap <= timeInBitnode;
    ...
    // Read per-sleeve dashboard overrides { [index]: taskSpec }
    let sleeveOverrides = {};
    try {
        const raw = ns.read('/Temp/sleeve-overrides.txt');
        if (raw && raw !== '') sleeveOverrides = JSON.parse(raw);
    } catch {}
    ...
    for (let i = 0; i < numSleeves; i++) {
        ...
        // Check for dashboard override — if one exists, use it instead of auto-picking
        let designatedTask, command, args, statusUpdate;
        const overrideSpec = sleeveOverrides[String(i)];
        if (overrideSpec) {
            [designatedTask, command, args, statusUpdate] = buildOverrideTask(i, overrideSpec);
        } else {
            [designatedTask, command, args, statusUpdate] =
                await pickSleeveTask(ns, playerInfo, playerWorkInfo, i, sleeve, canTrain, isBN10, trainingCapExpired);
        }
...
async function pickSleeveTask(ns, playerInfo, playerWorkInfo, i, sleeve, canTrain, isBN10 = false, trainingCapExpired = false) {
    ...
    if (canTrain) {
        ...
        // Only train if cap hasn't expired OR sleeve still has targets to hit
        if (!trainingCapExpired || untrainedStats.length > 0 || untrainedSmarts.length > 0) {
        ...
        } // !trainingCapExpired || has targets
    }
    ...
    // If gangs are available and shock is 0, prioritize homicide until we've got the requisite -54K karma.
    // Placed here (after training, before faction work) so shock==0 sleeves grind karma immediately
    ...
    // Multi-faction sleeve assignment: assign spare sleeves to different joined factions.
    // When in a gang, we still do this — the gang handles its own faction rep passively,
    // but sleeves can grind other factions. gangFaction is already excluded from availFactions.
    if (i != followPlayerSleeve && playerInfo.factions && playerInfo.factions.length > 0) {
