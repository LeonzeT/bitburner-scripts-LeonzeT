...
import { COMMON as C, ROUND1_CONSTANTS as R1} from '/corp/corp-constants.js';
// Names
function parseOptions(ns) {
    const defaults = Object.fromEntries(R1.argsSchema);
    const opts = { ...defaults };
    for (let i = 0; i < ns.args.length; i++) {
        ...
        const key = R1.ARG_ALIASES[rawKey] ?? rawKey;
        if (!(key in opts))
            continue;
...
export async function main(ns) {
    ...
    function getExperimentalRound1PrepCashReserve() {
        if (!useRound1Route())
            return 0;
        const buffer = C.ROUND1_ROUTE_PREP_RESERVE_BUFFER;
        try {
        ...
        try {
            if (c.getHireAdVertCount(C.DIV_AGRI) < getRound1AdvertTarget()) {
                return buffer + Math.max(0, Number(c.getHireAdVertCost(C.DIV_AGRI) ?? 0));
            }
        }
        catch { }
        try {
            ...
            for (const city of C.CITIES) {
                ...
                if (wh.level >= warehouseTarget)
                    continue;
                const cost = Number(c.getUpgradeWarehouseCost(C.DIV_AGRI, city, 1) ?? Infinity);
                if (Number.isFinite(cost))
                    cheapestWarehouse = Math.min(cheapestWarehouse, cost);
            }
    ...
    function noteRound1Gate(key, message, level = 'info') {
        if (!message)
            return;
        if (lastRound1GateNotes[key] === message)
            return;
    ...
    function getExperimentalRound1AgriBoostTargets(city, profile = 'dynamic') {
        try {
            if (!useRound1Route() || !hasDiv(C.DIV_AGRI) || !c.hasWarehouse(C.DIV_AGRI, city)) {
                return getBoostTargets(C.DIV_AGRI, city, AGRI_BOOST.factors, AGRI_BOOST.sizes, AGRI_BOOST.mats);
            }
            if (profile === 'initial') {
                const wh = c.getWarehouse(C.DIV_AGRI, city);
                return optimalBoosts(wh.size * C.ROUND1_ROUTE_FIXED_INITIAL_BOOST_USAGE_PCT, [...AGRI_BOOST.factors], [...AGRI_BOOST.sizes], [...AGRI_BOOST.mats]);
            }
    ...
    function getExperimentalRound1AgriPressureMetrics(city, profile = 'dynamic') {
        try {
            ...
            const productStoredSpace = (flow.foodStored * ROUND1_AGRI_PRODUCT_MAT_SIZES.Food) +
                (flow.plantsStored * ROUND1_AGRI_PRODUCT_MAT_SIZES.Plants);
            const currentInputSpace = (flow.waterStored * getMaterialSize('Water')) +
            ...
            const computeMetrics = (expected) => {
                const inputLeeway = profile === 'initial'
                    ? C.ROUND1_ROUTE_INITIAL_BOOST_INPUT_LEEWAY
                    : C.ROUND1_ROUTE_DYNAMIC_BOOST_INPUT_LEEWAY;
                const pressureBufferCycles = useRound1Route()
                    ? C.ROUND1_ROUTE_BOOST_PRESSURE_SUPPLY_BUFFER_CYCLES
                ...
                const baseTargetInputSpace = Object.entries(ROUND1_AGRI_REQUIRED).reduce((total, [mat, coeff]) => {
                    ...
                    const targetUnits = expected.perProduct *
                        coeff *
                        C.CYCLE_SECS *
                ...
                const targetInputSpace = profile === 'initial'
                    ? Math.max(C.ROUND1_ROUTE_INITIAL_BOOST_MIN_INPUT_SPACE, baseTargetInputSpace)
                    : baseTargetInputSpace;
                const prodCycles = profile === 'initial'
                    ? C.ROUND1_ROUTE_INITIAL_BOOST_PROD_CYCLES
                    : C.ROUND1_ROUTE_DYNAMIC_BOOST_PROD_CYCLES;
                const productSpikeSpace = ((expected.food * ROUND1_AGRI_PRODUCT_MAT_SIZES.Food) +
                    (expected.plants * ROUND1_AGRI_PRODUCT_MAT_SIZES.Plants)) *
                    C.CYCLE_SECS *
                    prodCycles *
                    C.ROUND1_ROUTE_DYNAMIC_BOOST_PROD_LEEWAY;
                const safetyPct = profile === 'initial'
                    ? C.ROUND1_ROUTE_INITIAL_BOOST_SAFETY_PCT
                    : C.ROUND1_ROUTE_DYNAMIC_BOOST_SAFETY_PCT;
                const safetySpace = Math.max(1, size * safetyPct);
                const maxTotalUsagePct = profile === 'initial'
                    ? C.ROUND1_ROUTE_INITIAL_BOOST_MAX_TOTAL_USAGE_PCT
                    : C.ROUND1_ROUTE_DYNAMIC_BOOST_MAX_TOTAL_USAGE_PCT;
                const nonBoostSpace = productStoredSpace + Math.max(currentInputSpace, targetInputSpace);
                const safeTotalUsageSpace = Math.min(size * maxTotalUsagePct, Math.max(0, size - productSpikeSpace - safetySpace));
            ...
            if (profile === 'initial') {
                for (let i = 0; i < 2; i++) {
                    const nextRatio = Math.min(C.ROUND1_ROUTE_INITIAL_BOOST_EXPECTED_RATIO_CAP, estimateExperimentalRound1AgriProjectedBoostRatio(city, metrics.boostSpace));
                    if (nextRatio <= projectedBoostRatio + 0.01)
                        break;
    ...
    function getExperimentalRound1AgriExpectedFlow(city) {
        try {
            ...
            const perProduct = Math.max(Number(flow.foodProduction ?? 0), Number(flow.plantsProduction ?? 0), hintedProd);
            return {
                perProduct,
    ...
    function estimateExperimentalRound1AgriProjectedBoostRatio(city, boostSpace) {
        try {
            const currentContribution = getDivisionCityBoostContribution(C.DIV_AGRI, city);
            const projectedTargets = optimalBoosts(Math.max(0, Number(boostSpace ?? 0)), [...AGRI_BOOST.factors], [...AGRI_BOOST.sizes], [...AGRI_BOOST.mats]);
            const projectedContribution = getDivisionCityBoostContribution(C.DIV_AGRI, city, projectedTargets);
        ...
        try {
            ...
            for (const city of C.CITIES) {
                try {
                    ...
                    if (!entry)
                        continue;
                    const addedSpace = getProjectedMaterialTargetAddedSpace(C.DIV_AGRI, city, entry.targets);
                    if (!Number.isFinite(addedSpace) || addedSpace <= 0)
                        continue;
            ...
            if (totalAddedSpace <= 0 || totalSize <= 0)
                return 0;
    ...
    function getExperimentalRound1InitialBoostBudget(reserve = 0) {
        try {
            if (!useRound1Route())
                return Math.max(0, Number(c.getCorporation().funds ?? 0) - reserve);
            return Math.max(0, Number(c.getCorporation().funds ?? 0) - reserve + C.ROUND1_ROUTE_INITIAL_BOOST_DEBT);
        }
        catch {
            return 0;
        }
    ...
    function estimateRound1AgriBoostFillSpend() {
        try {
            ...
            for (const city of C.CITIES) {
                if (!c.hasWarehouse(C.DIV_AGRI, city))
                    continue;
                total += estimateMaterialTargetSpend(C.DIV_AGRI, city, getBoostTargets(C.DIV_AGRI, city, AGRI_BOOST.factors, AGRI_BOOST.sizes, AGRI_BOOST.mats));
            }
            return total;
        }
        catch {
    ...
    function tryExperimentalRound1PreBoostSurplusSpend() {
        if (!useRound1Route() || !hasDiv(C.DIV_AGRI))
            return null;
        const prep = getRound1PrepStatus();
        if (!prep.complete)
            return null;
        const reserve = C.ROUND1_ROUTE_PREBOOST_CASH_FLOOR;
        const actions = [];
        try {
            while (true) {
                const options = [];
                try {
                    ...
                    if (level < C.ROUND1_ROUTE_PREBOOST_SURPLUS_SMART_FACTORIES_TARGET) {
                        options.push({
                ...
                try {
                    ...
                    if (level < C.ROUND1_ROUTE_PREBOOST_SURPLUS_SALES_BOTS_TARGET) {
                        options.push({
                ...
                try {
                    const count = c.getHireAdVertCount(C.DIV_AGRI);
                    if (count < C.ROUND1_ROUTE_PREBOOST_SURPLUS_ADVERT_TARGET) {
                        options.push({
                            cost: Number(c.getHireAdVertCost(C.DIV_AGRI) ?? Infinity),
                            key: 'adv',
                            perform: () => {
                                c.hireAdVert(C.DIV_AGRI);
                                actions.push(`Agriculture AdVert -> ${c.getHireAdVertCount(C.DIV_AGRI)}`);
                            },
                        });
                    }
                }
        ...
        try {
            while (c.getUpgradeLevel('FocusWires') < C.ROUND1_ROUTE_PREBOOST_SURPLUS_FOCUS_WIRES_TARGET) {
        ...
        try {
            while (c.getUpgradeLevel('Smart Storage') < C.ROUND1_ROUTE_PREBOOST_SURPLUS_SMART_STORAGE_TARGET) {
    ...
    //  Division helpers 
    function getRound1HighReinvestRealEstatePushPlan(materialFloor, usagePct = C.ROUND1_RE_PUSH_USAGE_PCT, maxSpendCap = Infinity) {
        ...
        if (candidates.length === 1) {
            ...
            if (!Number.isFinite(needed) || needed <= 0)
                return null;
            if (!Number.isFinite(spend) || spend < C.ROUND1_RE_PUSH_MIN_SPEND)
                return null;
            return {
                city: candidate.city,
        ...
        if (pushes.length <= 0)
            return null;
        if (totalSpend < C.ROUND1_RE_PUSH_MIN_SPEND)
            return null;
        return {
    ...
    function getRound1ReinvestBoostTopUpPlan(materialFloor, maxSpendCap = Infinity, avgUsageCap = C.ROUND1_REINVEST_BOOST_TOPUP_MAX_USAGE_PCT, peakUsageCap = C.ROUND1_REINVEST_BOOST_TOPUP_MAX_PEAK_PCT) {
        const funds = Number(c.getCorporation().funds ?? 0);
        const budget = Math.max(0, Math.min(funds - materialFloor, maxSpendCap));
        if (!Number.isFinite(budget) || budget <= 0)
            return null;
        const usage = getAgriWarehouseUsageSummary();
        if (usage.avg > avgUsageCap ||
        ...
        for (const city of C.CITIES) {
            try {
                ...
                for (const mat of C.AGRI_MATS) {
                    const cappedTarget = Math.min(Number(initialTargets?.[mat] ?? 0), Number(dynamicTargets?.[mat] ?? 0));
                    if (cappedTarget > 0)
                        targets[mat] = cappedTarget;
                }
                const spend = estimateMaterialTargetSpend(C.DIV_AGRI, city, targets);
                if (!Number.isFinite(spend) || spend <= 0)
                    continue;
        ...
        if (pending.length <= 0 || !Number.isFinite(projectedSpend) || projectedSpend <= 0)
            return null;
        const budgetScale = Math.min(1, budget / projectedSpend);
        const warehouseScale = getRound1HighReinvestBoostTopUpWarehouseScale(pending, avgUsageCap, peakUsageCap);
        ...
        for (const entry of pending) {
            const scaledTargets = scaleMaterialTargetsFromStored(C.DIV_AGRI, entry.city, entry.targets, scale);
            const spend = estimateMaterialTargetSpend(C.DIV_AGRI, entry.city, scaledTargets);
            if (!Number.isFinite(spend) || spend <= 1e6)
                continue;
            targetsByCity[entry.city] = scaledTargets;
            totalSpend += spend;
        }
        if (Object.keys(targetsByCity).length <= 0)
            return null;
        return {
            targetsByCity,
            spend: totalSpend,
    ...
    async function tryRound1HighReinvestRealEstatePush(materialFloor, usagePct = C.ROUND1_RE_PUSH_USAGE_PCT, maxSpendCap = Infinity) {
        const plan = getRound1HighReinvestRealEstatePushPlan(materialFloor, usagePct, maxSpendCap);
        if (!plan)
            return null;
        await waitUntilNotPurchase();
        for (const push of plan.pushes) {
            c.buyMaterial(C.DIV_AGRI, push.city, 'Real Estate', push.needed / C.CYCLE_SECS);
        }
        try {
            await waitForFreshPurchasePass();
        }
            ...
            for (const push of plan.pushes) {
                try {
                    c.buyMaterial(C.DIV_AGRI, push.city, 'Real Estate', 0);
                }
                catch { }
            }
        }
    ...
    async function tryRound1ReinvestBoostTopUp(materialFloor, maxSpendCap = Infinity, avgUsageCap = C.ROUND1_REINVEST_BOOST_TOPUP_MAX_USAGE_PCT, peakUsageCap = C.ROUND1_REINVEST_BOOST_TOPUP_MAX_PEAK_PCT) {
        const plan = getRound1ReinvestBoostTopUpPlan(materialFloor, maxSpendCap, avgUsageCap, peakUsageCap);
        if (!plan)
            return null;
        await applyBoostMaterialsBatch(C.DIV_AGRI, plan.targetsByCity, materialFloor);
        return plan;
    }
    function hasDiv(div) {
        try {
    ...
    function stopExperimentalRound1BoostTrim(cities = C.CITIES) {
        if (!hasDiv(C.DIV_AGRI))
            return;
        for (const city of cities) {
            round1ExperimentalBoostTrimSellRates[city] = {};
            for (const mat of C.ROUND1_ROUTE_BOOST_TRIM_ORDER) {
                try {
    ...
    function shouldPreserveExperimentalRound1Offer(bestOffer = 0, currentOffer = 0, stagnantChecks = 0) {
        ...
        return Number(bestOffer ?? 0) >= softFloor * C.ROUND1_ROUTE_BOOST_PRESERVE_SOFT_FLOOR_PCT &&
            Number(currentOffer ?? 0) >= Number(bestOffer ?? 0) * C.ROUND1_ROUTE_BOOST_PRESERVE_NEAR_BEST_PCT &&
            Number(stagnantChecks ?? 0) <= C.ROUND1_ROUTE_BOOST_PRESERVE_MAX_STAGNATION;
    }
    function manageExperimentalRound1BoostTrim(bestOffer = 0, currentOffer = 0, stagnantChecks = 0) {
        ...
        for (const city of C.CITIES) {
            try {
                ...
                for (const mat of C.ROUND1_ROUTE_BOOST_TRIM_ORDER) {
                    ...
                    const keepTarget = target * C.ROUND1_ROUTE_BOOST_PRETRIM_KEEP_RATIO;
                ...
                const hasRefillNeed = C.ROUND1_ROUTE_BOOST_TRIM_ORDER.some((mat) => {
                    const target = Math.max(0, Number(boostTargets[mat] ?? 0));
                    const stored = Math.max(0, Number(c.getMaterial(C.DIV_AGRI, city, mat).stored ?? 0));
                    return stored + 1 < target;
                });
                const nearBest = bestOffer <= 0 || currentOffer >= bestOffer * C.ROUND1_ROUTE_BOOST_REFILL_NEAR_BEST_PCT;
                const lowUsageWindow = usage <= C.ROUND1_ROUTE_BOOST_REFILL_START_PCT;
                ...
                const shouldRefill = !wasActive && hasRefillNeed && lowUsageWindow && nearBest &&
                    stagnantChecks >= C.ROUND1_ROUTE_BOOST_REFILL_STAGNATION && stateAllowsRefill;
                const wantsPretrim = !preserveOffer && hasTrimCapacity &&
                    (usage >= C.ROUND1_ROUTE_BOOST_PRETRIM_TRIGGER_USAGE_PCT ||
                        (usage >= C.ROUND1_ROUTE_BOOST_PRETRIM_MIN_USAGE_FOR_PREDICTIVE_PCT &&
                            predictedUsage >= C.ROUND1_ROUTE_BOOST_PRETRIM_TRIGGER_PREDICTIVE_PCT));
                const wantsFullTrim = !preserveOffer && hasTrimCapacity &&
                    (usage >= C.ROUND1_ROUTE_BOOST_TRIM_PRESSURE_PCT ||
                        (usage >= C.ROUND1_ROUTE_BOOST_FULL_TRIGGER_MIN_USAGE_PCT &&
                            predictedUsage >= C.ROUND1_ROUTE_BOOST_FULL_TRIGGER_PREDICTIVE_PCT));
                cityStates.push({
                    city,
                    wh,
                    pressure,
        ...
        for (const state of cityStates
            ...
            .slice(0, C.ROUND1_ROUTE_BOOST_TRIM_STAGGER_LIMIT)) {
            allowedActivationModes.set(state.city, 'pre');
        }
        for (const state of cityStates) {
            try {
                const { city, wh, pressure, usage, predictedUsage, boostTargets, wasMode, wasActive, hasRefillNeed, shouldRefill, wantsPretrim, wantsFullTrim, } = state;
                let mode = wasMode;
                if (preserveOffer) {
                    mode = 'off';
                }
                    ...
                    else if (usage <= C.ROUND1_ROUTE_BOOST_PRETRIM_RELEASE_PCT &&
                        predictedUsage < C.ROUND1_ROUTE_BOOST_PRETRIM_RELEASE_PREDICTIVE_PCT) {
                        mode = 'off';
                    }
                }
                else if (mode === 'full') {
                    if (!wantsFullTrim) {
                        if (wantsPretrim ||
                            usage > C.ROUND1_ROUTE_BOOST_PRETRIM_RELEASE_PCT ||
                            predictedUsage >= C.ROUND1_ROUTE_BOOST_PRETRIM_RELEASE_PREDICTIVE_PCT) {
                            mode = 'pre';
                        }
                        else {
                ...
                if (!active && !shouldRefill) {
                    ...
                    if (wasActive) {
                        events.push(`Agriculture ${city} ${wasMode === 'pre' ? 'boost pre-trim' : 'boost trim'} paused ` +
                ...
                if (shouldRefill) {
                    const flow = getAgriCityFlowNumbers(city);
                    const productSpikeSpace = ((flow.foodProduction * ROUND1_AGRI_PRODUCT_MAT_SIZES.Food) +
                        (flow.plantsProduction * ROUND1_AGRI_PRODUCT_MAT_SIZES.Plants)) *
                        C.CYCLE_SECS *
                        C.ROUND1_ROUTE_BOOST_REFILL_PROD_CYCLES *
                        C.ROUND1_ROUTE_BOOST_REFILL_PROD_LEEWAY;
                    const refillCapUsage = Math.min(Number(wh.size ?? 0) * C.ROUND1_ROUTE_BOOST_REFILL_USAGE_PCT, Math.max(0, Number(wh.size ?? 0) - productSpikeSpace));
                    let refillSpace = Math.max(0, refillCapUsage - Number(wh.sizeUsed ?? 0));
                    const refillNotes = [];
                    for (const mat of C.ROUND1_ROUTE_BOOST_TRIM_ORDER) {
                        const target = Math.max(0, Number(boostTargets[mat] ?? 0));
                        const stored = Math.max(0, Number(c.getMaterial(C.DIV_AGRI, city, mat).stored ?? 0));
                        const size = getPhysicalMaterialSize(mat, C.ROUND1_ROUTE_BOOST_TRIM_SIZES[mat] ?? 0);
                        ...
                        try {
                            c.buyMaterial(C.DIV_AGRI, city, mat, buyUnits > 0 ? buyUnits / C.CYCLE_SECS : 0);
                        }
                        catch { }
                        round1ExperimentalBoostTrimSellRates[city] ??= {};
                        round1ExperimentalBoostTrimSellRates[city][mat] = 0;
                        if (buyUnits > 0)
                            refillNotes.push(`${mat} +${Math.round(buyUnits)}`);
                    }
                    if (refillNotes.length > 0) {
                        events.push(`Agriculture ${city} boost refill active ` +
                            `(usage ${(usage * 100).toFixed(1)}%, pred ${(predictedUsage * 100).toFixed(1)}%, ` +
                ...
                const predictiveReservedSpace = Math.max(0, Number(pressure.productSpikeSpace ?? 0)) +
                    Math.max(0, Number(pressure.inputCatchupSpace ?? 0)) +
                    Math.max(0, Number(pressure.safetySpace ?? 0));
                const desiredUsedCap = isPretrim
                    ? Math.max(0, Math.min(warehouseSize * C.ROUND1_ROUTE_BOOST_PRETRIM_TARGET_USAGE_PCT, warehouseSize * C.ROUND1_ROUTE_BOOST_PRETRIM_TARGET_PREDICTIVE_PCT - predictiveReservedSpace))
                    : Math.max(0, Math.min(warehouseSize * C.ROUND1_ROUTE_BOOST_TRIM_RELEASE_PCT, warehouseSize * C.ROUND1_ROUTE_BOOST_TRIM_PREDICTIVE_PCT - predictiveReservedSpace));
                ...
                for (const mat of C.ROUND1_ROUTE_BOOST_TRIM_ORDER) {
                    const target = Math.max(0, Number(boostTargets[mat] ?? 0));
                    const keepTarget = Math.max(0, target * (isPretrim ? C.ROUND1_ROUTE_BOOST_TRIM_KEEP_RATIO : C.ROUND1_ROUTE_BOOST_TRIM_KEEP_RATIO));
                    const stored = Math.max(0, Number(c.getMaterial(C.DIV_AGRI, city, mat).stored ?? 0));
                    const size = getPhysicalMaterialSize(mat, C.ROUND1_ROUTE_BOOST_TRIM_SIZES[mat] ?? 0);
                ...
                if (remainingSpaceNeeded > 0 && totalSellableSpace > 0) {
                    ...
                    for (const entry of trimEntries) {
                        if (entry.size <= 0 || entry.sellableUnits <= 0)
                            continue;
            ...
            try {
                ...
                for (const [mat, coeff] of Object.entries(ROUND1_AGRI_REQUIRED)) {
                    ...
                    const initialSeed = initialSeedMode
                        ...
                        : 0;
                    const seed = Math.max(C.ROUND1_SUPPLY_SEED[mat] ?? 0, initialSeed);
                    const target = Math.max(seed, rawProd * coeff * C.CYCLE_SECS * bufferCycles);
        ...
        try {
            ...
            for (const city of C.CITIES) {
                ...
                for (const mat of C.ROUND1_ROUTE_BOOST_TRIM_ORDER) {
                    ...
                    cityActive = true;
                    activeRate += rate;
                    revenue += rate * getMaterialBuyPrice(C.DIV_AGRI, city, mat);
                }
                if (cityActive)
                    activeCities++;
            }
    ...
    function isRound1PrepBuiltOut() {
        ...
        for (const city of C.CITIES) {
            try {
                if (c.getWarehouse(C.DIV_AGRI, city).level < getRound1WarehouseTarget())
                    return false;
            }
            catch {
                return false;
        ...
        try {
            ...
            if (prep.advertCurrent < prep.advertTarget) {
                return {
                    ...
                    cost: Number(c.getHireAdVertCost(C.DIV_AGRI) ?? 0),
                };
            }
            if (prep.warehouseMin < prep.warehouseTarget) {
                ...
                for (const city of C.CITIES) {
                    try {
                        const wh = c.getWarehouse(C.DIV_AGRI, city);
                        if (Number(wh.level ?? 0) >= prep.warehouseTarget)
                            continue;
                        const cost = Number(c.getUpgradeWarehouseCost(C.DIV_AGRI, city, 1) ?? Infinity);
    ...
    function shouldUseRound1Stretch(bestOffer, stagnantChecks) {
        if (!useRound1Route())
            return false;
        return bestOffer >= C.ROUND1_REINVEST_STRETCH_TRIGGER ||
            stagnantChecks >= C.ROUND1_REINVEST_STRETCH_STAGNATION;
    }
    function getRound1ReinvestReserve(bestOffer) {
        ...
        if (progress >= getRound1FreezeRatio())
            return Math.max(24e9, funds * 0.72);
        if (progress >= 0.82)
            return Math.max(16e9, funds * 0.55);
        if (progress >= 0.65)
            return Math.max(10e9, funds * 0.40);
        return Math.max(C.ROUND1_REINVEST_RESERVE_MIN, funds * 0.25);
    }
    function advanceRound1Prep(smartStorageTarget, warehouseTarget, advertTarget) {
        const funds = Math.max(0, Number(c.getCorporation().funds ?? 0));
        const prepBuffer = useRound1Route() ? C.ROUND1_ROUTE_PREP_RESERVE_BUFFER : 0;
        if (funds > 2e9)
            try {
        ...
        if (funds > 3e9)
            try {
                while (c.getHireAdVertCount(C.DIV_AGRI) < advertTarget
                    && c.getCorporation().funds > c.getHireAdVertCost(C.DIV_AGRI) + prepBuffer) {
                    c.hireAdVert(C.DIV_AGRI);
                }
            }
            catch { }
        if (funds > 1e9)
            for (const city of C.CITIES)
                try {
                    while (c.getWarehouse(C.DIV_AGRI, city).level < warehouseTarget
                        && c.getCorporation().funds > c.getUpgradeWarehouseCost(C.DIV_AGRI, city, 1) + prepBuffer) {
    ...
    async function investInAgricultureForRound1(bestOffer, stagnantChecks, currentOffer = bestOffer) {
        if (bestOffer < C.ROUND1_REINVEST_TRIGGER &&
            stagnantChecks < C.ROUND1_REINVEST_TRIGGER_STAGNATION) {
            return {
                status: 'trigger-wait',
                message: `Round-1 route waiting for reinvest trigger - best ${formatMoney(bestOffer)} / stagnant ${stagnantChecks}, ` +
                    `needs ${formatMoney(C.ROUND1_REINVEST_TRIGGER)} or stagnation ${C.ROUND1_REINVEST_TRIGGER_STAGNATION}.`,
            };
        }
        const stretch = shouldUseRound1Stretch(bestOffer, stagnantChecks);
        const officeTarget = stretch ? C.ROUND1_REINVEST_OFFICE_STRETCH : C.ROUND1_REINVEST_OFFICE;
        const warehouseTarget = stretch ? C.ROUND1_REINVEST_WAREHOUSE : getRound1WarehouseTarget();
        const advertTarget = stretch ? C.ROUND1_REINVEST_ADVERT_STRETCH : C.ROUND1_REINVEST_ADVERT;
        const smartFactoriesTarget = stretch ? C.ROUND1_REINVEST_SMART_FACTORIES_STRETCH : C.ROUND1_REINVEST_SMART_FACTORIES;
        const smartStorageTarget = Math.max(getRound1SmartStorageTarget(), stretch ? C.ROUND1_REINVEST_SMART_STORAGE - 1 : C.ROUND1_REINVEST_SMART_STORAGE);
        const reserve = getRound1ReinvestReserve(bestOffer);
        const materialFloor = -(stretch ? C.ROUND1_REINVEST_MATERIAL_DEBT_STRETCH : C.ROUND1_REINVEST_MATERIAL_DEBT);
        const maxDebtPushSpend = stretch ? C.ROUND1_RE_PUSH_MAX_SPEND_STRETCH : C.ROUND1_RE_PUSH_MAX_SPEND;
        const corpFunds = () => Number(c.getCorporation().funds ?? 0);
        const actions = [];
        let actionsTaken = 0;
        let capacityActionsTaken = 0;
        let debtPushGateReason = '';
        const canTakeAction = (cost, capacityAction = false) => actionsTaken < C.ROUND1_REINVEST_MAX_ACTIONS &&
            (!capacityAction || capacityActionsTaken < C.ROUND1_REINVEST_MAX_CAPACITY_ACTIONS) &&
            Number.isFinite(cost) &&
            corpFunds() - cost >= reserve;
        const recordAction = (capacityAction = false) => {
            actionsTaken++;
            if (capacityAction)
                capacityActionsTaken++;
        };
        for (const city of C.CITIES) {
            try {
                ...
                if ((off.size ?? 0) < targetSize) {
                    ...
                    if (canTakeAction(cost)) {
                        fillOffice(C.DIV_AGRI, city, nextSize, getRound2AgriProductionJobs(nextSize));
                        recordAction();
                        actions.push(`Agriculture ${city} office -> ${nextSize}`);
                    }
                    else {
                        fillOffice(C.DIV_AGRI, city, off.size, getRound2AgriProductionJobs(off.size));
                    }
                }
                else {
                    fillOffice(C.DIV_AGRI, city, off.size, getRound2AgriProductionJobs(off.size));
            ...
            try {
                const wh = c.getWarehouse(C.DIV_AGRI, city);
                if (wh.level < warehouseTarget) {
                    const spend = estimateWarehouseUpgradeSpend(C.DIV_AGRI, city);
        ...
        try {
            while (c.getHireAdVertCount(C.DIV_AGRI) < advertTarget &&
                actionsTaken < C.ROUND1_REINVEST_MAX_ACTIONS) {
        ...
        if (actionsTaken < C.ROUND1_REINVEST_MAX_ACTIONS) {
            const usage = getAgriWarehouseUsageSummary();
            const topUpWindowOpen = (bestOffer >= C.ROUND1_REINVEST_BOOST_TOPUP_TRIGGER &&
                stagnantChecks >= R1.ROUND1_REINVEST_BOOST_TOPUP_MIN_STAGNATION) ||
                (stagnantChecks >= C.ROUND1_REINVEST_BOOST_TOPUP_STAGNATION &&
                    bestOffer >= C.ROUND1_REINVEST_BOOST_TOPUP_TRIGGER *
                        C.ROUND1_REINVEST_BOOST_TOPUP_STAGNATION_TRIGGER_PCT);
            const bridgeTopUpWindowOpen = !topUpWindowOpen &&
                stagnantChecks >= R1.ROUND1_REINVEST_BRIDGE_TOPUP_MIN_STAGNATION &&
                bestOffer >= C.ROUND1_REINVEST_BRIDGE_TOPUP_TRIGGER &&
                currentOffer >= bestOffer * C.ROUND1_REINVEST_BRIDGE_TOPUP_NEAR_BEST_PCT &&
                usage.avg <= C.ROUND1_REINVEST_BRIDGE_TOPUP_MAX_USAGE_PCT &&
                usage.peak <= C.ROUND1_REINVEST_BRIDGE_TOPUP_MAX_PEAK_PCT;
            if (round1ReinvestDebtSettleChecks > 0) {
            ...
            else if (currentOffer < bestOffer * C.ROUND1_RE_PUSH_OFFER_PCT) {
                debtPushGateReason =
                    `waiting for offer recovery (${formatMoney(currentOffer)} vs best ${formatMoney(bestOffer)})`;
            }
            else if (topUpWindowOpen) {
                const boostTopUp = await tryRound1ReinvestBoostTopUp(materialFloor, maxDebtPushSpend, C.ROUND1_REINVEST_BOOST_TOPUP_MAX_USAGE_PCT, C.ROUND1_REINVEST_BOOST_TOPUP_MAX_PEAK_PCT);
                if (boostTopUp) {
                    round1ReinvestDebtSettleChecks = C.ROUND1_RE_PUSH_SETTLE_CHECKS;
                    recordAction();
                ...
                const bridgeBoostTopUp = await tryRound1ReinvestBoostTopUp(materialFloor, Math.min(maxDebtPushSpend, C.ROUND1_REINVEST_BRIDGE_TOPUP_MAX_SPEND), C.ROUND1_REINVEST_BRIDGE_TOPUP_MAX_USAGE_PCT, C.ROUND1_REINVEST_BRIDGE_TOPUP_MAX_PEAK_PCT);
                if (bridgeBoostTopUp) {
                    round1ReinvestDebtSettleChecks = C.ROUND1_RE_PUSH_SETTLE_CHECKS;
                    recordAction();
            ...
            else if (bestOffer >= C.ROUND1_REINVEST_BOOST_TOPUP_TRIGGER &&
                stagnantChecks < R1.ROUND1_REINVEST_BOOST_TOPUP_MIN_STAGNATION) {
                debtPushGateReason =
                    `offer still rising, waiting for stagnation before top-up ` +
                        `(${stagnantChecks}/${C.ROUND1_REINVEST_BOOST_TOPUP_MIN_STAGNATION})`;
            }
            else if (bestOffer >= C.ROUND1_REINVEST_BRIDGE_TOPUP_TRIGGER &&
                stagnantChecks < R1.ROUND1_REINVEST_BRIDGE_TOPUP_MIN_STAGNATION) {
                debtPushGateReason =
                    `offer still rising, waiting for stagnation before bridge top-up ` +
                        `(${stagnantChecks}/${C.ROUND1_REINVEST_BRIDGE_TOPUP_MIN_STAGNATION})`;
            }
            else if (!topUpWindowOpen) {
                debtPushGateReason =
                    `waiting for late boost top-up window (${formatMoney(bestOffer)} vs trigger ` +
                        `${formatMoney(C.ROUND1_REINVEST_BOOST_TOPUP_TRIGGER)})`;
            }
            else {
        ...
        const smartStorageCurrent = (() => { try {
            return Number(c.getUpgradeLevel('Smart Storage') ?? 0);
        }
        catch {
            return 0;
        } })();
        const smartFactoriesCurrent = (() => { try {
            return Number(c.getUpgradeLevel('Smart Factories') ?? 0);
        }
        catch {
            return 0;
        } })();
        const advertCurrent = (() => { try {
            return Number(c.getHireAdVertCount(C.DIV_AGRI) ?? 0);
        }
        catch {
            return 0;
        } })();
        const minWarehouse = getMinWarehouseLevel(C.DIV_AGRI);
        const minOffice = getMinOfficeSize(C.DIV_AGRI, C.CITIES);
        const targetsMet = minOffice >= officeTarget &&
            ...
            try {
                ...
                if ((off.size ?? 0) < targetSize) {
                    ...
                    pendingCosts.push({
                        ...
                        cost: c.getOfficeSizeUpgradeCost(C.DIV_AGRI, city, increase),
                    });
                }
            }
            catch { }
            try {
                ...
                if (wh.level < warehouseTarget) {
                    pendingCosts.push({
                        ...
                        cost: estimateWarehouseUpgradeSpend(C.DIV_AGRI, city),
                    });
                }
            }
            catch { }
        ...
        try {
            if (advertCurrent < advertTarget) {
                pendingCosts.push({
                    ...
                    cost: c.getHireAdVertCost(C.DIV_AGRI),
                });
            }
        }
        catch { }
        ...
        const debtPushCandidate = getRound1ReinvestBoostTopUpPlan(materialFloor, maxDebtPushSpend);
        return {
    ...
    if (phase <= 0) {
        if (!c.hasCorporation()) {
            log(ns, `INFO: Creating "${C.CORP_NAME}"...`, true, 'info');
            const inBn3 = ns.getResetInfo().currentNode === 3;
            if (!c.createCorporation(C.CORP_NAME, !inBn3 || opts['self-fund'])) {
        ...
        for (const name of [C.UNLOCKS.warehouseAPI, C.UNLOCKS.officeAPI])
            buyUnlock(name);
    ...
    if (phase <= 1) {
        ...
        if (!c.getCorporation().divisions.includes(C.DIV_AGRI)) {
            ns.tprint(`INFO: Round1 expanding ${C.DIV_AGRI}`);
            log(ns, 'INFO: Expanding into Agriculture ($40B)...', true, 'info');
            c.expandIndustry(C.IND_AGRI, C.DIV_AGRI);
            ns.tprint(`INFO: Round1 expanded ${C.DIV_AGRI}`);
        }
        expandToCities(C.DIV_AGRI);
        ns.tprint(`INFO: Round1 expanded ${C.DIV_AGRI} to cities`);
        log(ns, `INFO: Round-1 route pre-boost warehouse step - raising Agriculture warehouses to ${C.ROUND1_ROUTE_PREBOOST_WAREHOUSE} during initial setup.`, true, 'info');
        bulkUpgradeWarehousesToLevel(C.DIV_AGRI, C.ROUND1_ROUTE_PREBOOST_WAREHOUSE);
        ...
        for (const city of C.CITIES)
            fillOffice(C.DIV_AGRI, city, 4, { rnd: 4 });
        for (const city of C.CITIES) {
            try {
        ...
        advanceRound1Prep(phase1Round1SmartStorageTarget, phase1Round1WarehouseTarget, phase1Round1AdvertTarget);
        const initialSurplus = tryExperimentalRound1PreBoostSurplusSpend();
        if (initialSurplus?.actions?.length) {
            noteRound1Gate('round1-preboost-surplus', `Round-1 route pre-boost surplus spend: ${initialSurplus.actions.join(', ')} ` +
                `(cash floor ${formatMoney(initialSurplus.reserve)}, funds ${formatMoney(initialSurplus.funds)}).`);
        }
        maintainRound1AgriSupply(C.CITIES, getExperimentalRound1PrepCashReserve(), C.CYCLE_SECS, 'initial');
        await waitCycles(1);
        ...
        while (c.getDivision(C.DIV_AGRI).researchPoints < 55) {
            bulkUpgradeWarehousesToLevel(C.DIV_AGRI, C.ROUND1_ROUTE_PREBOOST_WAREHOUSE);
            advanceRound1Prep(phase1Round1SmartStorageTarget, phase1Round1WarehouseTarget, phase1Round1AdvertTarget);
            const surplus = tryExperimentalRound1PreBoostSurplusSpend();
            if (surplus?.actions?.length) {
                noteRound1Gate('round1-preboost-surplus', `Round-1 route pre-boost surplus spend: ${surplus.actions.join(', ')} ` +
        ...
        for (const city of C.CITIES)
            assignJobs(C.DIV_AGRI, city, { ops: 1, eng: 1, biz: 1, mgmt: 1 });
        advanceRound1Prep(phase1Round1SmartStorageTarget, phase1Round1WarehouseTarget, phase1Round1AdvertTarget);
        const postRpSurplus = tryExperimentalRound1PreBoostSurplusSpend();
        if (postRpSurplus?.actions?.length) {
            noteRound1Gate('round1-preboost-surplus', `Round-1 route pre-boost surplus spend: ${postRpSurplus.actions.join(', ')} ` +
                `(cash floor ${formatMoney(postRpSurplus.reserve)}, funds ${formatMoney(postRpSurplus.funds)}).`);
        }
        stopRound1AgriSupply(C.CITIES);
        maintainRound1AgriSupply(C.CITIES, getExperimentalRound1PrepCashReserve(), C.CYCLE_SECS, 'initial');
        await waitCycles(1);
        stopRound1AgriSupply(C.CITIES);
        ...
        const initialBoostMaterialFloor = -C.ROUND1_ROUTE_INITIAL_BOOST_DEBT;
        ...
        for (const city of C.CITIES) {
            ...
            projectedSpend += estimateMaterialTargetSpend(C.DIV_AGRI, city, targets);
        }
        ...
        initialBoostScale = Math.min(initialBoostScale, C.ROUND1_ROUTE_INITIAL_BOOST_SCALE_CAP);
        if (initialBoostScale < 0.999) {
            noteRound1Gate('round1-initial-fill-budget', `Round-1 route initial Agri boost budget cap active - scaling phase-1 fill to ` +
                `${(initialBoostScale * 100).toFixed(1)}% (budget ${formatMoney(budget)}, projected spend ${formatMoney(projectedSpend)}, ` +
                `debt floor ${formatMoney(-C.ROUND1_ROUTE_INITIAL_BOOST_DEBT)}).`);
        }
        const initialBatchTargets = Object.fromEntries(C.CITIES.map((city) => [
            city,
            scaleMaterialTargets(initialBoostTargetsByCity[city] ?? getExperimentalRound1AgriBoostTargets(city, 'initial'), initialBoostScale),
        ]));
        const initialBatch = await applyBoostMaterialsBatchChunked(C.DIV_AGRI, initialBatchTargets, initialBoostMaterialFloor, C.ROUND1_ROUTE_INITIAL_BOOST_CHUNK_FRACTION, C.ROUND1_ROUTE_INITIAL_BOOST_MAX_PASSES);
        if ((initialBatch?.passes ?? 0) > 1) {
            noteRound1Gate('round1-initial-fill-execution', `Round-1 route initial Agri boost fill executed over ${initialBatch.passes} purchase passes ` +
                `(chunk ${(C.ROUND1_ROUTE_INITIAL_BOOST_CHUNK_FRACTION * 100).toFixed(0)}%).`);
        }
        maintainRound1AgriSupply(C.CITIES, 0, 2 * C.CYCLE_SECS, 'initial');
        writePhase(2);
        phase = 2;
    }
    // 
    ...
    if (phase <= 2) {
        ...
        log(ns, `INFO: Round-1 route mode enabled - targeting ${formatMoney(round1Target)} with ` +
            `plateau accept at ${formatMoney(round1SoftFloor)}, plus post-prep Agri reinvest with bounded material debt before the final freeze.`, true, 'info');
        let bestOffer = 0;
        let stagnantChecks = 0;
        let spendingFrozen = false;
        let phase2PrepDone = isRound1PrepBuiltOut();
        // Pre-seed warehouse capacity so refreshBoosts doesn't fire for all cities on the
        // first iteration after a script restart (prevWHCapacity starts empty each run).
        for (const city of C.CITIES)
            try {
                prevWHCapacity[`${C.DIV_AGRI}|${city}`] = c.getWarehouse(C.DIV_AGRI, city).size;
            }
            catch { }
        while (true) {
            const lateRound1Window = spendingFrozen ||
                bestOffer >= round1SoftFloor * C.ROUND1_ROUTE_LATE_WINDOW_SOFT_FLOOR_PCT;
            await waitCycles(lateRound1Window ? 1 : 2);
            let round1ActionState = '';
            const startupSupplyProfile = !spendingFrozen && bestOffer < C.ROUND1_REINVEST_TRIGGER
                ? 'startup'
                : 'dynamic';
            ...
            maintainRound1AgriSupply(C.CITIES, !phase2PrepDone ? getExperimentalRound1PrepCashReserve() : 0, lateRound1Window ? C.CYCLE_SECS : 2 * C.CYCLE_SECS, startupSupplyProfile);
            const offer = c.getInvestmentOffer();
            const offerBasis = (() => {
                try {
            ...
            if (bestOffer >= round1Target * getRound1FreezeRatio())
                spendingFrozen = true;
            if (!spendingFrozen) {
                if (!phase2PrepDone) {
                    ...
                    if (!phase2PrepDone) {
                        const prep = getRound1PrepStatus();
                        noteRound1Gate('round1-prep', `Round-1 route prep waiting on fixed setup - ${prep.missing.join(', ')}.`);
                    }
                }
                else {
                    ...
                    if (expRound1?.status === 'acted') {
                        noteRound1Gate('round1-spend', `Round-1 route spend: ${expRound1.actions.join(', ')} ` +
                            `(funds ${formatMoney(expRound1.funds)}, reserve ${formatMoney(expRound1.reserve)}).`);
                    }
                    else if (expRound1?.message) {
                        noteRound1Gate(expRound1.status === 'trigger-wait'
                            ? 'round1-trigger'
                            : 'round1-spend', expRound1.message);
                    }
                }
            }
            else {
            ...
            if (spendingFrozen) {
                ...
                if (pressureEvents?.length) {
                    noteRound1Gate('round1-pressure', `Round-1 route pressure relief: ${pressureEvents.join(', ')}`);
                }
            }
            ...
            log(ns, `  Round ${offer.round} debug: ${formatRound1Debug(c.getCorporation(), offer.funds ?? 0, bestOffer, stagnantChecks, spendingFrozen, phase2PrepDone, round1ActionState, offerBasis)}`, false);
            if (offer.round > 1) {
                log(ns, 'INFO: Round 1 already accepted.', true, 'info');
                break;
            }
            ...
            const acceptExperimentalFrozenNearBest = offer.round === 1 &&
                spendingFrozen &&
                stagnantChecks >= C.ROUND1_ROUTE_FROZEN_ACCEPT_MIN_STAGNATION &&
                bestOffer >= round1Target * C.ROUND1_ROUTE_FROZEN_ACCEPT_TARGET_PCT &&
                offer.funds >= Math.max(round1SoftFloor, bestOffer * C.ROUND1_ROUTE_FROZEN_ACCEPT_NEAR_BEST_PCT);
            if (acceptExperimentalFrozenNearBest) {
                c.acceptInvestmentOffer();
                lockBn3HighBudgetRound2Profile(Number(offer.funds ?? 0));
                log(ns, `INFO: Accepted Round 1 near-best freeze window received ${formatMoney(offer.funds)} ` +
                    `(best ${formatMoney(bestOffer)}).`, true, 'success');
                break;
            }
