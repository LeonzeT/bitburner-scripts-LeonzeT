...
export async function main(ns) {
    ...
    // Upgrade baseline captured at R3 phase-7 entry. Used to guarantee R3 always
    // buys at least C.PRIVATE_STAGE_MIN_UPGRADE_DELTA above whatever R2 left off.
    let privateStageBaseUpgradeLevels = null;
    ...
    function readLock() {
        try {
            return JSON.parse(ns.read(C.SETUP_LOCK) || 'null');
        }
        catch {
            return null;
        }
    ...
    function acquireLock() {
        if (lockValid(readLock()))
            return false;
        ns.write(C.SETUP_LOCK, JSON.stringify({
    ...
    function getPrivateStageUpgradeTarget(stageName, name) {
        ...
        switch (name) {
            case 'Smart Factories':
                staticTarget = targets.smartFactories;
                break;
            case 'Smart Storage':
                staticTarget = targets.smartStorage;
                break;
            case 'ABC SalesBots':
                staticTarget = targets.salesBots;
                break;
            case 'Wilson Analytics':
                staticTarget = targets.wilson;
                break;
            default: return 0;
        }
        // Always buy at least delta more than R2 left off, so upgrades
        // never silently skip because R2's dynamic late spend already passed
        // the static target.
        if (privateStageBaseUpgradeLevels !== null) {
            const base = privateStageBaseUpgradeLevels[name] ?? 0;
            const delta = R3.PRIVATE_STAGE_MIN_UPGRADE_DELTA[stageName]?.[name] ?? 0;
            return Math.max(staticTarget, base + delta);
        }
        return staticTarget;
    }
    function capturePrivateStageBaseUpgradeLevels() {
        if (privateStageBaseUpgradeLevels !== null)
            return;
        const levels = {};
        for (const name of ['Smart Factories', 'Smart Storage', 'ABC SalesBots', 'Wilson Analytics']) {
            try {
                levels[name] = c.getUpgradeLevel(name);
            }
            catch {
                levels[name] = 0;
            }
        }
        privateStageBaseUpgradeLevels = levels;
    }
    ...
    function tryPrivateStageEmployeeUpgrade(stageName, reserve) {
        const target = C.PRIVATE_STAGE_EMPLOYEE_UPGRADE_TARGETS[stageName] ?? 0;
        if (target <= 0)
            return null;
    ...
    //  Handoff 
    if (phase <= 7) {
        capturePrivateStageBaseUpgradeLevels();
