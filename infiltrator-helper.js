...
export async function main(ns) {
    ...
    if (flags.trade)   ns.tprint("  auto-trade enabled.");
...
function _infTick() {
    try {
        ...
        if (!_onAnyInfiltrationScreen()) {
            ...
            // 4. Auto-restart: look for an "Infiltrate" button and click it.
            //    A short cooldown prevents rapid repeated clicks.
...
function _onAnyInfiltrationScreen() {
    ...
    for (const el of doc.querySelectorAll("h4"))
        if (el.textContent.trim().startsWith("Infiltrating")) return true;
    return false;
}
...
function _handleVictoryScreen() {
    // The victory screen's h4 reads "Infiltration successful!"
    let onVictory = false;
    for (const el of doc.querySelectorAll("h4"))
    ...
    for (const btn of doc.querySelectorAll("button")) {
        ...
        if (wnd._infAutoSell  && txt.startsWith("sell for"))  { btn.click(); return true; }
        if (wnd._infAutoTrade && txt.startsWith("trade for") && !btn.disabled) { btn.click(); return true; }
    }
    // If neither button was matched (e.g. trade=true but faction="none"), do nothing
    // but still signal we're on the victory screen so the tick doesn't loop uselessly.
    return true;
}
...
function _findInfiltrateButton() {
    for (const btn of doc.querySelectorAll("button"))
        if (btn.textContent.trim().toLowerCase() === "infiltrate") return btn;
    return null;
}
