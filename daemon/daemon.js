...
let _ns = null; // Set by main() before any nested calls so resolveScript can use it
// Module-level script path resolver. Takes only (key) so it can be called from
// any nested function (doTargetingLoop, startup, etc.) without passing ns.
// _scriptPaths is populated lazily on first call via the cached ns reference.
// The inner 'const resolveScript = key => ...' inside startup() shadows this
// harmlessly — both do the same thing.
function resolveScript(key) {
    if (!_scriptPaths) {
        _scriptPaths = {};
        try {
            // Use the cached ns reference (_ns set in main before any nested calls)
            const r = _ns?.read('/script-paths.json');
            if (r && r !== '') { _scriptPaths = JSON.parse(r); delete _scriptPaths._comment; }
        } catch {}
    }
    if (typeof key !== 'string') return key; // guard against undefined/null
...
const argsSchema = [
    ...
    ['disable-hacknet', false], // Set to true to completely skip purchasing/upgrading hacknet nodes. Useful when hacknet ROI is too slow relative to your BN goals.
    ...
    ['skip-hwgw-exec-hosts', true], // If true, read /Temp/hwgw-exec-hosts.txt and exclude those servers from script distribution.
                                    // Set to false if you want daemon.js to use all servers regardless of hwgw-manager's claims.
...
export async function main(ns) {
    _ns = ns; // Cache ns for module-level helpers (resolveScript etc.)
    // --- CONSTANTS ---
    ...
    let cachedHwgwExecHosts = new Set(); // Exec hosts claimed by hwgw-manager; refreshed each doTargetingLoop iteration
    ...
    async function startup(ns) {
        ...
        // Resolve registered script paths via script-paths.json (0 GB — ns.read only)
        let _paths = {};
        try { const r = ns.read('/script-paths.json'); if (r && r !== '') { _paths = JSON.parse(r); delete _paths._comment; } } catch {}
        const resolveScript = key => _paths[key] ?? (key.endsWith('.js') ? key : key + '.js');
        ...
        ownedCracks = [];
        psCache = {};
        cachedHwgwExecHosts = new Set(); // Reset on startup; populated each doTargetingLoop iteration
        ...
        const shouldUpgradeHacknet = () =>
            !options['disable-hacknet'] && // Can be disabled via --disable-hacknet flag
        ...
        asynchronousHelpers = [
            ...
            { name: "go.js", shouldRun: () => reqRam(64), minRamReq: 20.2, shouldTail: false }, // Play go.js (various multipliers, but large dynamic ram requirements)
            { name: resolveScript('stockmaster'), shouldRun: () => reqRam(64), args: openTailWindows ? ["--show-market-summary"] : [] }, // Start our stockmaster
            ...
            { name: resolveScript('sleeve'), shouldRun: () => reqRam(64) && 10 in dictSourceFiles }, // Script to create manage our sleeves for us
            { name: resolveScript('gangs'), shouldRun: () => reqRam(64) && 2 in dictSourceFiles }, // Script to create manage our gang for us
            {
                name: resolveScript('work-for-factions'), args: ['--fast-crimes-only', '--no-coding-contracts'],  // Singularity script to manage how we use our "focus" work.
                ...
                shouldTail: false, // autopilot manages work-for-factions separately; no need for a tail window here
            },
            {
                name: resolveScript('bladeburner'), // Script to manage bladeburner for us. Run automatically if not disabled and bladeburner API is available
                shouldRun: () => !options['disable-script'].includes(resolveScript('bladeburner')) && reqRam(64)
        ...
        periodicScripts = [
            ...
            { interval: 26000, name: "/Tasks/program-manager.js",
                // Keep running until all darkweb programs are bought (port crackers + utilities + DarkscapeNavigator)
                shouldRun: () => 4 in dictSourceFiles && allHostNames.includes("darkweb") &&
                    !["BruteSSH.exe","FTPCrack.exe","relaySMTP.exe","HTTPWorm.exe","SQLInject.exe",
                      "ServerProfiler.exe","DeepscanV1.exe","DeepscanV2.exe","AutoLink.exe",
                      "Formulas.exe","DarkscapeNavigator.exe"].every(p => ns.fileExists(p, "home")) },
            ...
            {   // Periodically look to purchase new servers, but note that these are often not a great use of our money (hack income isn't everything) so we may hold-back.
                interval: 32000, name: resolveScript('host-manager'), minRamReq: 6.55,
    ...
    async function runPeriodicScripts(ns) {
        ...
        if (9 in dictSourceFiles && !options['disable-spend-hashes'] && !options['disable-hacknet']) { // See if we have a hacknet, and spending hashes for money isn't disabled
    ...
    async function doTargetingLoop(ns) {
        ...
        do {
            ...
            try {
                ...
                // Read hwgw exec hosts once per loop, cache for use in both getNetworkStats
                // and arbitraryExecution below. Reading once avoids repeated ns.read() calls.
                const prevHwgwExecHosts = cachedHwgwExecHosts;
                cachedHwgwExecHosts = new Set();
                if (options['skip-hwgw-exec-hosts']) {
                    try {
                        const raw = ns.read('/Temp/hwgw-exec-hosts.txt');
                        if (raw && raw !== '') cachedHwgwExecHosts = new Set(JSON.parse(raw));
                    } catch { /* not running yet */ }
                }

                // Evict daemon's own scripts from newly-claimed exec hosts.
                // Daemon stops LAUNCHING to claimed servers via the arbitraryExecution filter,
                // but any /Remote/ scripts that were already running there before the partition
                // was established continue to occupy RAM and conflict with hwgw workers.
                // When we see a host appear in the claimed set for the first time (or any time
                // daemon restarts and inherits a pre-claimed partition), kill daemon's tools on it.
                if (cachedHwgwExecHosts.size > 0) {
                    for (const claimedHost of cachedHwgwExecHosts) {
                        if (prevHwgwExecHosts.has(claimedHost)) continue; // Already knew about this one
                        // Newly seen claimed host — kill any daemon /Remote/ scripts running there.
                        // All daemon worker scripts live under /Remote/ (weak-target, grow-target,
                        // hack-target, share). We don't need a tool list — the path prefix is enough.
                        let evicted = 0;
                        for (const proc of ns.ps(claimedHost)) {
                            if (proc.filename.startsWith('/Remote/')) {
                                ns.kill(proc.pid);
                                evicted++;
                            }
                        }
                        if (evicted > 0)
                            log(ns, `INFO: Evicted ${evicted} daemon script(s) from hwgw-claimed host "${claimedHost}".`);
                    }
                }

                // If hwgw-manager is active, suppress daemon's /Remote/ hack scheduling entirely.
                // hwgw-batcher is orders of magnitude more efficient, and daemon's tools racing
                // for the same servers cause the eviction/refill stall that kills the batcher pipeline.
                // Clearing targetingOrder here prevents any weaken/grow/hack batches from being
                // scheduled this loop — daemon keeps doing everything else (rooting, hacknet,
                // share, XP farming, stockmaster, work-for-factions) unaffected.
                if (options['skip-hwgw-exec-hosts'] && cachedHwgwExecHosts.size > 0) {
                    targetingOrder = [];
                } else if (options['skip-hwgw-exec-hosts']) {
                    // hwgw not active — still filter out any targets it's explicitly claimed
                    // (in case exec hosts file exists but manager just restarted)
                    try {
                        const hwgwStatus = ns.read('/Temp/hwgw-status.txt');
                        if (hwgwStatus && hwgwStatus !== '') {
                            const hwgwTargets = new Set(JSON.parse(hwgwStatus).targets ?? []);
                            if (hwgwTargets.size > 0)
                                targetingOrder = targetingOrder.filter(s => !hwgwTargets.has(s.name));
                        }
                    } catch { /* hwgw-manager not running yet — skip silently */ }
                }
                ...
                if (xpOnly) { // If all we want to do is gain hack XP
                    ...
                    // Skip XP farming when hwgw-manager owns the purchased servers.
                    // daemon sees low utilization (hwgw workers aren't daemon workers) but
                    // getMaxThreads() finds near-zero free RAM → threads==0 → noisy warning.
                    cachedHwgwExecHosts.size === 0 &&
                    !processList(ns, daemonHost).some(p => p.filename.endsWith(resolveScript('hwgw-manager')))) {
                ...
                // Skip sharing if hwgw-manager is running — it owns the purchased servers and
                // is actively earning money with them. Daemon's visible RAM is only home +
                // world servers, so "low utilization" here doesn't mean RAM is truly free.
                // Sharing that small slice would reduce hack income for negligible rep gain.
                const hwgwManagerRunning = cachedHwgwExecHosts.size > 0 ||
                    processList(ns, daemonHost).some(p => p.filename.endsWith(resolveScript('hwgw-manager')));
                ...
                if (failed.length <= 0 && utilizationPercent < maxShareUtilization && // Only share RAM if we have succeeded in all hack cycle scheduling and have RAM to space
                    ...
                    !hwgwManagerRunning && // Don't share when hwgw-manager owns purchased servers — it earns money with them
    ...
    function getNetworkStats() {
        ...
        // Exclude hwgw exec hosts from free RAM stats. Their RAM is claimed by hwgw workers
        // and unavailable to daemon's arbitraryExecution. If we include their (currently empty)
        // RAM in totalFreeRam, getTotalNetworkUtilization() stays near 0% during prep even
        // though daemon can't actually use that RAM — causing isWorkCapped() to never trigger
        // and daemon to repeatedly attempt scheduling and log "WARNING: Scheduling failed".
        const executableServers = rootedServers.filter(s => !cachedHwgwExecHosts.has(s.name));
        const listOfServersFreeRam = executableServers.map(s => s.ramAvailable()).filter(ram => ram > 1.6); // Servers that can't run a script don't count
        const totalMaxRam = executableServers.map(s => s.totalRam()).reduce((a, b) => a + b, 0);
    ...
    async function arbitraryExecution(ns, tool, threads, args, preferredServerName = null, useSmallestServerPossible = false, allowThreadSplitting = null) {
        // Block /Remote/weak-target, /Remote/grow-target, /Remote/hack-target, and
        // /Remote/manualhack-target when hwgw-manager has claimed exec hosts.
        // /Remote/share.js is always allowed — it runs on unclaimed RAM and doesn't
        // interfere with the batcher pipeline.
        // We re-read the file directly rather than trusting cachedHwgwExecHosts because
        // the cache is cleared-then-repopulated at the start of each loop tick, so any
        // call arriving during that window would slip through if we used the cache.
        const REMOTE_BLOCKED = [
            "/Remote/weak-target.js", "/Remote/grow-target.js",
            "/Remote/hack-target.js", "/Remote/manualhack-target.js",
        ];
        if (REMOTE_BLOCKED.includes(tool.name)) {
            if (!ns.fileExists(tool.name, daemonHost)) return false; // deleted — skip silently
            try {
                const raw = ns.read('/Temp/hwgw-exec-hosts.txt');
                if (raw && raw !== '') return false; // hwgw active — block
            } catch { /* file missing means hwgw not running — allow through */ }
        }
        ...
        // If hwgw-manager is running, it claims purchased servers as exclusive worker exec hosts.
        // We respect that claim by filtering those servers out of our own distribution lists.
        // This prevents daemon.js scripts from competing for RAM with hwgw batch workers.
        // Controlled by --skip-hwgw-exec-hosts (default: true). Set to false to disable.
        if (options['skip-hwgw-exec-hosts']) {
            // Use the loop-level cached set rather than re-reading the file here.
            // cachedHwgwExecHosts is refreshed once per doTargetingLoop iteration.
            if (cachedHwgwExecHosts.size > 0) {
                const notClaimed = server => !cachedHwgwExecHosts.has(server.name);
                rootedServersByFreeRam.splice(0, rootedServersByFreeRam.length,
                    ...rootedServersByFreeRam.filter(notClaimed));
                preferredServerOrder.splice(0, preferredServerOrder.length,
                    ...preferredServerOrder.filter(notClaimed));
            }
        }
