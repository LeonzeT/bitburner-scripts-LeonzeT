...
let lastIntTravel = 0; // Total money EARNED at last INT-farming city travel (triggers every 10M earned)
...
export async function main(ns) {
    ...
    // Fast path: read the cached source file dict written by a prior run.
    // In BN13 (SF4.1), ns.singularity.getOwnedSourceFiles() costs 80 GB — more
    // than available RAM. The cache at this path is written by the same temp script
    // on any successful prior call (e.g. from daemon, autopilot, or a prior wff run
    // with more free RAM). Using it here avoids the RAM spike entirely.
    try {
        const cached = ns.read('/Temp/getOwnedSourceFiles-asDict.txt');
        if (cached && cached !== '' && !cached.startsWith('ERROR') && cached !== '<Insufficient RAM>') {
            const parsed = JSON.parse(cached);
            if (4 in parsed) {
                dictSourceFiles = parsed;
                log(ns, 'INFO: Using cached source file data (SF4.' + parsed[4] + ' confirmed).');
            }
        }
    } catch {}
...
async function loadStartupData(ns) {
    // Helper: read a Temp cache file if it exists and is fresh (< maxAgeMs old),
    // otherwise fall back to the expensive getNsDataThroughFile call.
    // Reduces peak RAM in BN13/SF4.1 where singularity costs ×16.
    const readCacheOrFetch = async (cacheFile, fetchCmd, fetchArgs = [], maxAgeMs = 60000) => {
        try {
            const raw = ns.read(cacheFile);
            if (raw && raw !== '' && raw !== '<Insufficient RAM>' && !raw.startsWith('ERROR')) {
                const parsed = JSON.parse(raw);
                // Check for _ts metadata (written by dashboard companions)
                const age = parsed?._ts ? Date.now() - parsed._ts : Infinity;
                // If no _ts, assume fresh if the file has valid data (wff itself wrote it)
                if (age < maxAgeMs || !parsed._ts) return parsed;
            }
        } catch {}
        return await getNsDataThroughFile(ns, fetchCmd, cacheFile, fetchArgs);
    };
    ...
    // Use cached files when fresh — avoids 48-80 GB temp scripts in BN13/SF4.1
    dictFactionFavors = await readCacheOrFetch('/Temp/getFactionFavors.txt', dictCommand('ns.singularity.getFactionFavor(o)'), allKnownFactions, 90000);
    const dictFactionAugs = await readCacheOrFetch('/Temp/getAugmentationsFromFactions.txt', dictCommand('ns.singularity.getAugmentationsFromFaction(o)'), allKnownFactions, 120000);
    const augmentationNames = [...new Set(Object.values(dictFactionAugs).flat())];
    const dictAugRepReqs = await readCacheOrFetch('/Temp/getAugmentationRepReqs.txt', dictCommand('ns.singularity.getAugmentationRepReq(o)'), augmentationNames, 3600000);
    const dictAugStats = await readCacheOrFetch('/Temp/getAugmentationStats.txt', dictCommand('ns.singularity.getAugmentationStats(o)'), augmentationNames, 3600000);
    // Clear all aug-related cache files before reading so we never use stale data
    // from the previous install cycle. faction-manager and ascend.js write these files
    // right before installing with every aug marked as owned — if work-for-factions reads
    // them before they're refreshed it sees -1 for every faction's aug requirement.
    // null filename → /Temp/ns-singularity-getOwnedAugmentations.txt (auto-derived, fixed)
    // We must explicitly clear it to force the temp script to re-execute.
    ns.write('/Temp/ns-singularity-getOwnedAugmentations.txt', '', 'w');
    ns.write('/Temp/player-augs-purchased.txt', '', 'w');
    ns.write('/Temp/player-augs-installed.txt', '', 'w');
    ...
    // Also skip softCompletedFactions (all desired augs owned) — no point joining or
    // working for them when there's nothing we want to buy with the rep.
    const gangSkip = (playerGang && !firstFactions.includes(playerGang)) ? [playerGang] : [];
    skipFactions = options.skip.concat(cannotWorkForFactions).concat(completedFactions).concat(softCompletedFactions).concat(gangSkip).filter(fac => !firstFactions.includes(fac));
    // DIAGNOSTIC: show owned aug count and what's being skipped so we can see if cache is stale
    ns.print(`[DIAG] ownedAugmentations count: ${ownedAugmentations.length} (installed: ${installedAugmentations.length})`);
    ns.print(`[DIAG] Sample owned: ${ownedAugmentations.slice(0,5).join(', ') || '(none)'}`);
    ...
    // Show any joined factions NOT in skip (should be what we work for)
    const joinedAndActive = playerInfo.factions.filter(f => !skipFactions.includes(f) && f !== playerGang);
    ns.print(`[DIAG] Joined factions NOT skipped (will work for): ${joinedAndActive.join(', ') || '(none)'}`);
...
async function mainLoop(ns) {
    ...
    // Throttle: checkFactionInvitations costs 48 GB in BN13/SF4.1 — don't call every 5s loop
    if (!mainLoop.lastInviteCheck || Date.now() - mainLoop.lastInviteCheck > 30000)
        mainLoop.lastInviteCheck = Date.now();
    else
        mainLoop.skipInviteCheck = true;
    const invites = mainLoop.skipInviteCheck ? [] : await checkFactionInvites(ns);
    mainLoop.skipInviteCheck = false;
    ...
    // If autopilot's XP cycle is active, Chongqing's Neuregen (1.4× hacking_exp) is
    // the single most impactful aug to unlock. Bump it to position 2 in the work order
    // so it's prioritised right after Netburners (which we always want first for hashes).
    const xpCycleActive = (() => {
        try { const f = ns.read('/Temp/xp-grind-active.txt'); return f && f.trim() !== ''; } catch { return false; }
    })();
    if (xpCycleActive && priorityFactions.includes('Chongqing') && !priorityFactions.includes('Aevum')) {
        // Only move Chongqing up if Aevum isn't joined (they're mutually exclusive city factions)
        priorityFactions = priorityFactions.filter(f => f !== 'Chongqing');
        const netburnersIdx = priorityFactions.indexOf('Netburners');
        priorityFactions.splice(netburnersIdx + 1, 0, 'Chongqing');
        ns.print('INFO: XP cycle active — prioritising Chongqing for Neuregen (1.4× hacking_exp)');
    }

    // INT farming: travel to a new city every 10M EARNED (not held) for +3 INT exp per trip.
    // Uses ns.getMoneySources().sinceInstall.total so reserve.txt holdings don't interfere.
    // Costs $200k per trip — safe since threshold is 10M earned.
    // Skipped if casino.js is running (it manages city travel independently).
    const intTravelMoneyThreshold = 10e6;
    const moneySrc = await getNsDataThroughFile(ns, 'ns.getMoneySources()');
    const totalEarned = Object.values(moneySrc?.sinceInstall ?? {}).reduce((a, v) => a + (v || 0), 0);
    if (totalEarned - lastIntTravel > intTravelMoneyThreshold) {
        const casinoRunning = ns.ps().some(p => p.filename.includes('casino.js'));
        if (!casinoRunning) {
            const cities = ['Sector-12', 'Aevum', 'Volhaven', 'Chongqing', 'New Tokyo', 'Ishima'];
            const otherCities = cities.filter(c => c !== player.city);
            const targetCity = otherCities[Math.floor(totalEarned / intTravelMoneyThreshold) % otherCities.length];
            if (await goToCity(ns, targetCity)) {
                lastIntTravel = totalEarned;
                log(ns, `INT travel: moved to ${targetCity} for INT exp (+3). Total earned: ${formatMoney(totalEarned)}`, false, 'info');
            }
        } else {
            lastIntTravel = totalEarned; // casino.js running, skip travel but advance threshold
        }
    }
    ...
    // Exclude completedFactions and softCompletedFactions — nothing to buy from them.
    // (completedFactions: all augs owned. softCompletedFactions: all desired augs owned.)
    // This prevents the 5%-rep grind from running against factions the user has finished.
    let factionsWeCanWorkFor = joinedFactions.filter(f => !options.skip.includes(f) && !cannotWorkForFactions.includes(f) && f != playerGang && !completedFactions.includes(f) && !softCompletedFactions.includes(f));
    let foundWork = false;
...
async function earnFactionInvite(ns, factionName) {
    ...
    if (['Tian Di Hui', 'Tetrads', 'The Dark Army'].includes(factionName)) {
        // Tetrads/Tian Di Hui accept Chongqing, New Tokyo, or Ishima.
        // If already in one of these cities, no travel needed — just wait for invite.
        const validCities = ['Chongqing', 'New Tokyo', 'Ishima'];
        const alreadyInValidCity = validCities.includes(player.city);
        if (alreadyInValidCity)
            travelledForInvite = true; // Already there — the invite condition is met, just wait for it
        else
...
export async function crimeForKillsKarmaStats(ns, reqKills, reqKarma, reqStats, doFastCrimesOnly = false) {
    const bestCrimesByDifficulty = ["Homicide"]; // Will change crimes as our success rate improves
    ...
    while (forever || (needStats = anyStatsDeficient(player)) || player.numPeopleKilled < reqKills || -ns.heart.break() < reqKarma) {
        ...
        // Homicide is always optimal when karma or kills are needed:
        // Even at 0% success, failed Homicide gives karma/4 = 0.75 per 3s = 0.25/s,
        // which is 4x better than Mug at 100% success (0.25/4s = 0.0625/s).
        // Homicide also builds all combat stats, so needStats is not a reason to avoid it.
        crime = (player.numPeopleKilled < reqKills || karma < reqKarma)
            ? "Homicide" // Always Homicide when kills or karma needed, regardless of success chance
            : bestCrimesByDifficulty.find((c, index) => doFastCrimesOnly && index <= 1 ? 0 : crimeChances[c] >= chanceThresholds[index]) ?? "Homicide"; // stat-building: pick by success threshold
