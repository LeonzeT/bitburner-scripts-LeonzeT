...
export async function main(ns) {
    ...
    // Early exit if Stanek's Gift is effectively disabled in this BN (e.g. BN8: StaneksGiftExtraSize=-99)
    const giftHeight = await getNsDataThroughFile(ns, 'ns.stanek.giftHeight()');
    const giftWidth  = await getNsDataThroughFile(ns, 'ns.stanek.giftWidth()');
    if (giftHeight < 2 || giftWidth < 2) {
        return log(ns, `INFO: Stanek's Gift is disabled or too small in this BitNode (grid: ${giftWidth}×${giftHeight}). Exiting.`, true);
    }
    ...
    idealReservedRam = options['reserved-ram-ideal']; // RAM to reserve when it's a small fraction of available RAM (was hardcoded to 32, ignoring the flag)
...
async function tryChargeAllFragments(ns, fragmentsToCharge) {
    ...
    return true; // All fragments charged successfully this cycle
}
