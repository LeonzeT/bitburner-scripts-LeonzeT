...
async function initAllStocks(ns) {
    ...
    return allStockSymbols.map(s => ({
		...
		expectedReturn: function () {
			...
			return 2 * this.vol * conservativeProb;  // was: this.vol * conservativeProb
		},
...
async function updateForecast(ns, allStocks, has4s) {
	...
	// Write a lightweight flag so hwgw-batcher.js can skip stock manipulation
    // when we hold no open positions (avoids nudging forecasts for no benefit).
    await ns.write('/Temp/stock-has-positions.txt',
        allStocks.some(stk => stk.sharesLong > 0 || stk.sharesShort > 0) ? 'true' : 'false', 'w');
}
