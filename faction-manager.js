...
const default_priority_augs = [
    ...
    "Neuregen", // Chongqing-exclusive: 1.4× hacking_exp — highest hack XP multiplier in the game
]; // By default, take these augs when they are accessible
...
export async function main(ns) {
    ...
    if ((desiredStatsFilters?.length ?? 0) == 0) { // If the user does has not specified stats or augmentations to prioritize, use sane defaults
        ...
        desiredStatsFilters = willTakeAnyAug ? ['*'] : // Take any aug if one of the above criteria is met
            // When donations are free (favorToDonate===0, e.g. BN8 or any future BN with the same property),
            // only bother installing if it boosts hack level/exp — rep multipliers are
            // irrelevant since you can buy rep directly without favor.
            favorToDonate === 0 ? ['hacking_level', 'hacking_exp'] :
    ...
    if (ignorePlayerData)
        ...
        if (!forceJoinFactions?.length) {
            ...
            if (!gangFaction && 2 in ownedSourceFiles && ns.heart.break() <= -53000) {
                forceJoinFactions.push(...potentialGangFactions); // spread — not push(array) which creates a nested array
...
async function joinFactions(ns, forceJoinFactions) {
    ...
    for (const faction of invitations.sort(factionSortOrder)) {
        ...
        // Use unownedAugs (not newAugs) for the desired check — we want to join any faction
        // that has unowned desired augs, even if those same augs are theoretically accessible
        // from another already-joined faction. Having multiple rep paths costs nothing and
        // avoids silently skipping factions (e.g. NiteSec) whose augs overlap with others.
        // Null-guard augmentationData[aug] in case aug info wasn't fetched for this faction.
        let desiredAugs = unownedAugs.filter(aug => augmentationData[aug]?.desired);
...
async function managePurchaseableAugs(ns, outputRows, accessibleAugs) {
    ...
    // 4S TIX API BUDGET GUARD — do not let NF purchases consume money reserved for
    // 4S Market Data / TIX API access. Stocks are our primary income in most BNs,
    // and buying 4S is almost always more valuable per dollar than extra NF levels.
    //
    // We check whether 4S data TIX API has been purchased. If not, we compute its
    // total cost (Data + API, accounting for BN multipliers) and reduce the available
    // NF budget by that amount, effectively ring-fencing that money so stockmaster
    // can acquire 4S without competing with NF purchases.
    //
    // This guard is bypassed in BN8 (stocks-heavy BN where stockmaster is the main
    // income path) after the player has already joined Daedalus — at that stage speed
    // of installing more augs matters more than preserving 4S budget, since in BN8
    // donations unlock immediately and rep is easy to regrind.
    let nfBudget = budget;
    try {
        const has4SApi  = await getNsDataThroughFile(ns, 'ns.stock.has4SDataTixApi()', '/Temp/has-4s-api.txt');
        if (!has4SApi) {
            const has4SData = await getNsDataThroughFile(ns, 'ns.stock.has4SData()', '/Temp/has-4s-data.txt');
            const apiCost  = 25e9  * (bitNodeMults?.FourSigmaMarketDataApiCost  ?? 1);
            const dataCost = has4SData ? 0 : 1e9 * (bitNodeMults?.FourSigmaMarketDataCost ?? 1);
            const reserveFor4S = apiCost + dataCost;
            if (reserveFor4S > 0 && nfBudget > reserveFor4S) {
                nfBudget -= reserveFor4S;
                const in4SBn = resetInfo?.currentNode == 8;
                if (!in4SBn || !playerData?.factions?.includes('Daedalus'))
                    log(ns, `INFO: Reserving ${formatMoney(reserveFor4S)} for 4S TIX API (reducing NF budget from ` +
                        `${formatMoney(budget)} to ${formatMoney(nfBudget)})`, false);
            }
        }
    } catch {
        // Stock API may not be accessible yet in very early game — skip guard silently
    }
    ...
    while (augNfFaction && nfPurchased < 200) { // Limit to 200 to avoid breaking the game if near infinite money.
        ...
        if (totalCostWithNextNf > nfBudget || nextNfRep > augNfFaction.reputation && !augNfFaction.donationsUnlocked) {
            nextUpNf = `Next NF (L${nextNfLevel}) will be available at:`.padEnd(37) +
                ...
                `${(totalCostWithNextNf > nfBudget ? '✗' : '✓')} and ${formatNumberShort(nextNfRep)} Reputation with "${augNfFaction.name}" (` +
